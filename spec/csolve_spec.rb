	...
	describe "::read_congruence" do
		context "when input contains no '\='" do
			it "raises an ArgumentError" do
				...
				expect {PolynomialInterpreter.read_congruence not_congruence}.to raise_error ArgumentError, "not a congruence"
			end
		end
		...
		context "when LHS is empty" do
			it "raises an ArgumentError" do
				congruence = "= 3 (mod 4)"
				expect {PolynomialInterpreter.read_congruence congruence}.to raise_error ArgumentError, "LHS polynomial blank"
			end
		end
		...
		context "when RHS is empty" do
			it "raises an ArgumentError" do
				...
				expect {PolynomialInterpreter.read_congruence congruence}.to raise_error ArgumentError, "RHS polynomial blank"
			end
		end
		...
		context "when input is a valid polynomial congruences" do
			it "formats congruence as a single polynomial congruent to 0" do
				congruence = "45x^5 + 5 + 3x ^6 + 5x^2 + x + 3x^5=x^9 + 9 x (mod16)"
